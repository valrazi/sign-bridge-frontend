...
const getDetail = async () => {
    if (id) {
        try {
            ...
            if (!response.error) {
                const { data } = response
                payload.value.backgroundImage = data.backgroundImage
                payload.value.content = data.content
                payload.value.title = data.title
                v$.value.$touch()
                thumbnail.value = data.backgroundImage
            }
        } catch (error) {
            console.log(error)
        } finally {
...
const submit = async () => {
    ...
    if (!v$.value.$invalid) {
        ...
        try {
            let media = ''
            if(!originalFile) {
                media = payload.value.backgroundImage 
            }else {
                media = (await apiMediaUpload(payload.value.backgroundImage)).data
            }
            const response = await apiBlogEdit({
...
<template>
    ...
    <div class="w-full flex flex-col justify-center items-center gap-y-4 py-8" v-if="id">
    ...
    <div class="w-full h-dvh flex justify-center items-center" v-else>
